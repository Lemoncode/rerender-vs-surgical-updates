...
const convertToHex = (hsl: { h: number; s: number; l: number }) => {
  const { h, s, l } = hsl;
...
export const setPrimaryColor = (hex: string) => {
  const hsl = convertToHSL(hex);

  document.documentElement.style.setProperty("--primary-main-h", `${hsl.h}`);
  document.documentElement.style.setProperty("--primary-main-s", `${hsl.s}%`);
  document.documentElement.style.setProperty("--primary-main-l", `${hsl.l}%`);
};
